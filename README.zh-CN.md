...
- `project` 且**未在** checklist 执行中（含仍在 planning）时，确认链以 **自审完成** 结束（不勾选推进）；RUN 执行中仍按项推进/完成。**Stop dirty-arm**（编辑钩子未触发时的 git 产品脏路径）在 **`project`** 下同样适用于 ambient idle+armed / planning / executing — 不只限于 RUN；**completed 且无 session** + 产品脏可 **ensure** ambient 再开 fix。**`executing_only`** 下 dirty-arm / ensure ambient 仍仅 RUN。
...
- [宿主说明](./docs/hosts.md)（Cursor / Claude Code / Codex / Kimi Code / Copilot CLI / Grok Build / Gemini CLI / Factory Droid / Hermes Agent / Antigravity / Pi / Devin CLI 已支持；Kimi 为 **Stop≤1/turn 降级**；Copilot 为 **Stop consecutive ≤8 降级**；Grok 为 **Stop ≤8/turn 降级**；Gemini **AfterAgent ≤100** / min-CLI **≥0.31.0**；Factory **multi-block 活链已证**；Hermes **pre_verify continue 活链已证**；Antigravity **活链已证** Stop continue（0.10.1）；Pi **Shipped** TUI 活链 continue ≥1× 已证 — 进程内扩展，非 shell hook stamp；Devin CLI **Shipped** 交互 CLI 活链 continue ≥1× 已证 — **仅 CLI**，不测 Desktop；**Runner = Shipped (meta)**；**OpenCode = 1 (next)**（等上游）；完整扩宿主路线图）  
