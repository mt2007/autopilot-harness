...
Track checklists live under **`artifacts.plans_dir`** (`<plansDir>/<slug>/checklist.md`). Do not assume the literal path `plans/`.
...
2. **Executing** — only when this conversation is already `phase=executing` for the armed track (never treat needPick / pending selection as executing):
   ...
   - Read `<plansDir>/<slug>/checklist.md`; implement the first unchecked item.
