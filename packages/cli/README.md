...
Bin name: `autopilot-harness`. Requires **Node.js 22+**. **This build ships Cursor, Claude Code, Codex, Kimi Code, GitHub Copilot CLI, and Grok Build CLI** (Kimi is **degraded Stop≤1/turn**; Copilot is **degraded Stop consecutive ≤8**; Grok is **degraded Stop ≤8/turn**).
...
# or: --platform claude-code | --platform codex | --platform kimi-code | --platform copilot-cli | --platform grok-build
...
Reload the host (Cursor: Reload Window; Claude Code / Codex / Kimi Code / Copilot CLI / Grok Build: restart / new session). **Restart Copilot CLI** after install or upgrade so hooks reload. Trust Codex via `/hooks` and Grok via `/hooks-trust` or `--trust` when applicable, then:

1. Plan — Cursor/Claude: `/autopilot-on`; Codex / Kimi Code / Copilot CLI / Grok Build: line-start `triggers.on` (typed slash still parses)
2. Execute — Cursor/Claude: `/autopilot-run`; Codex / Kimi Code / Copilot CLI / Grok Build: line-start `triggers.run`
