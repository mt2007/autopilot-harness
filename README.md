...
- Under **`project`**, confirm still runs the same lenses; when you are **not** checklist-executing (idle ambient, or still **planning**), the chain ends with **review complete** (local commit if dirty) — it does **not** advance/check off checklist items. While **RUN** + executing, the same lenses still end in checklist **advance** / **done** as usual. **Stop dirty-arm** (git-dirty product paths when an edit hook did not fire) also applies under **`project`** for ambient idle+armed / planning / executing — not only during RUN; completed Stop with **no session** + product dirty may **ensure** an ambient session then open fix. Under **`executing_only`**, dirty-arm / ensure ambient stay RUN-only.
