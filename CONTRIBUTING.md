...
After changing `packages/templates/` (skills / workflows / `.autopilotignore`), run `pnpm bundle-templates` (or `pnpm build`) so `packages/cli/assets/templates/` stays in sync. The published tarball ships `dist/` only (`files: ["dist"]`); `pnpm build` / root `pnpm test` run `sync-dist-assets` (`assets/` → `dist/assets/`) before pack.
