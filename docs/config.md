...
**What actually reads which keys (this Cursor + Claude Code + Codex + Kimi Code + Copilot CLI + Grok Build CLI build):**
...
| `platforms` | `[{ id: cursor, surface: ide }]` | Enabled hosts (`surface`: `ide` \| `cli` \| `runner`). Cap: 32 unique bindings. **Primary** = first installable binding in list order (no separate primary key). **This build installs Cursor, Claude Code, Codex, Kimi Code, Copilot CLI, and/or Grok Build CLI** when those bindings are present. Claude/Codex/Kimi/Copilot/Grok use `surface: cli` (Claude hooks shared across terminal + IDE — not CLI-only; Codex: `.codex/hooks.json` + `/hooks` trust; Kimi: user-home `config.toml`, **Stop≤1/turn** degraded; Copilot: `.github/hooks/autopilot-harness.json`, **Stop consecutive ≤8** degraded, restart CLI after install; Grok: `.grok/hooks/autopilot-harness.json`, **Stop ≤8/turn** degraded, trust via `/hooks-trust` or `--trust`). Multi-host: `init --yes --add-platform <host>`. Installed Autopilot hook commands include `--platform <id>` for six-way dispatch. |
...
| File | Role |
|------|------|
| **`.autopilotignore`** | Gitignore-style globs: matching edits do **not** count as product code (do not open fix→confirm). Missing file → built-in defaults (`plans/**`, `.autopilot/**`, `.cursor/**`, `.claude/**`, `.codex/**`, `.github/hooks/**`, `.grok/hooks/**`, `node_modules/**`, …). Does **not** change `git status` / `git diff`. |
...
- [Troubleshooting](./troubleshooting.md) — `doctor` WARNs, double hooks, missing skills, Claude `BLOCK_CAP` / trust, Copilot Stop≤8 / restart / dual, Grok Stop≤8/turn / trust / multi-FP  
- [Hosts](./hosts.md) — Cursor / Claude Code / Codex / Kimi Code / Copilot CLI / Grok Build (shipped; Kimi **degraded Stop ≤1/turn**; Copilot **degraded Stop consecutive ≤8**; Grok **degraded Stop ≤8/turn**); roadmap + stop-loop caps  
