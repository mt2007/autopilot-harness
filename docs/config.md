...
| Key | Default | Meaning |
|-----|---------|---------|
| `runner.command` | *(empty / commented example only)* | **Required** for `npx @autopilot-harness/cli runner start` with the real CliDriver. Template may include `{prompt}` and/or `{prompt_file}`. Treat as **trusted project config** (CliDriver spawns argv with `shell: false` — not a shell string, but still an executable you chose). Init **never** writes a runnable fake command — missing/blank (incl. whitespace-only after trim) → start **FAIL** + doctor **WARN** (not FAIL). |
...
| `runner.prompt_mode` | `auto` | `argv` \| `file` \| `auto` (long tips may use `{prompt_file}`). Invalid value → start **FAIL**. |
