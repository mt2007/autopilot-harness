...
CLI: `runner start` (`--run [slug]`, `--conversation`, flags override YAML) and `runner status`. With pending/executing and **not paused**, **resume without `--run`**. **Paused** → start **FAIL** (bare or `--run`) until unpaused. Bare `--run` (no slug) when **several** tracks are runnable → **needPick** (process **exit 2**, print candidates; not a doctor line) — then `--run <slug>`. **`--on` / planning-in-runner is deferred** (v1) — plan with a hook host first. See [Hosts — Runner](./hosts.md#status).
