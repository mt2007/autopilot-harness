...
Autopilot separates **core** (FSM, SQLite, checklist, review) from **ports** (host adapters). **This build ships Cursor, Claude Code, Codex, and Kimi Code** (CLI + App share one Codex hook surface; Kimi is a **degraded** Stop-continue port — **Stop≤1/turn**). Further hosts are on the [roadmap](#roadmap-not-shipped) below.

Installed hook commands stamp **`--platform <id>`** (e.g. `cursor` / `claude-code` / `codex` / `kimi-code`) as the primary dispatch switch (**quaternary** today). Vendor runtime exports **aliased** Codex and Kimi handlers (`handleCodexUserPromptSubmit` / `handleCodexPostToolUse` / `handleCodexStop` / `handleKimiUserPromptSubmit` / `handleKimiPostToolUse` / `handleKimiStop`) so they never collide with Claude’s bare names. Runtime still applies **universal abort** and a **payload conflict resolver** so IDE cross-fire (Cursor-shaped stdin on a Claude/Codex/Kimi-stamped command) cannot recover into `decision:block`.
...
|---------:|------|-----|----------------|
| **1 (next)** | **GitHub Copilot CLI** | High | `agentStop` supports `{ decision:"block", reason }` + `stop_hook_active`; runaway guard ≈ **8** consecutive blocks (same class of problem as Claude `BLOCK_CAP` — need a raise/disable path or document chunking). Large audience. |
| **2** | **Grok Build CLI** | High | Stop can keep the agent working; may load Claude/Cursor hook files. **Do not** assume Claude JSON output is honored — verify Grok’s top-level `decision` contract and whether `UserPromptSubmit` can inject / block. |
| **3** | **Gemini CLI** | High | Rich agent hooks (`BeforeAgent` / `AfterAgent` with retry/halt). Different event names than Claude — more adapter work, strong product reach. |
| **4** | **Factory Droid** | Medium–High | Project/user `.factory/hooks.json` (Claude-shaped events). Confirm Stop (or equivalent) can force another turn with a reason string. |
| **5** | **Hermes Agent** | Medium | Shell/plugin hooks; `pre_verify` accepts Claude Stop shape (`decision:"block"` + `reason`) as continue, capped by `agent.max_verify_nudges` (default **3**). Verify gate is edit-triggered — map carefully onto Autopilot’s always-on stop loop. |
| **6** | **Antigravity** | Medium | `.agents/hooks.json` / `~/.gemini/config/hooks.json` with `PreToolUse` / `PostToolUse` / `Stop`. Confirm Stop can **inject continue** (not observe-only) before committing a port. |
| **7** | **OpenCode** | Medium–Low | Extensibility is **plugin**-centric; first-party Stop-continue is weaker than Claude/Codex. Community Claude-compat plugins exist but are partial — prefer waiting for a stable first-party contract or use Runner. |
| **8** | **Runner** | Meta | Catch-all for hosts that cannot stop-continue (or when we want one external loop). Stub exists; ship after at least one more native hook port **or** when targeting a no-hooks host. |
...
Submit / edit hooks (and Claude/Codex/Kimi `UserPromptSubmit` analogues) are **not** subject to Cursor’s stop `loop_limit`.
...
- [Troubleshooting](./troubleshooting.md) — missing `loop_limit` / `BLOCK_CAP`, Codex trust / timeout, Kimi Stop≤1/turn, double hooks  
