...
Autopilot Harness separates **core** (FSM, SQLite, checklist, review) from **ports** (Cursor, Claude Code, Codex, Kimi Code, GitHub Copilot CLI, Grok Build CLI, Gemini CLI, Factory Droid, Hermes Agent, Antigravity, Pi, Devin CLI, Runner, …).
...
`platforms`; **this build installs Cursor, Claude Code, Codex, Kimi Code, Copilot CLI, Grok Build CLI, Gemini CLI, Factory Droid, Hermes Agent, and/or Antigravity, and/or Devin CLI, and/or Pi** (hook / in-process hosts) **and/or Runner** when those
bindings are present (`cursor`/`ide`, `claude-code`/`cli`, `codex`/`cli`, `kimi-code`/`cli`, `copilot-cli`/`cli`, `grok-build`/`cli`, `gemini-cli`/`cli`, `factory-droid`/`cli`, `hermes-agent`/`cli`, `antigravity`/`cli`, `devin`/`cli`, `pi`/`cli`, `runner`/`runner`). Other ids are
...
Copilot CLI uses `surface: cli` (project `.github/hooks/autopilot-harness.json`; **Restart Copilot CLI** after install/upgrade). Grok Build CLI uses `surface: cli` (project `.grok/hooks/autopilot-harness.json`; **trust** via `/hooks-trust` or `--trust`; reload / new session after install/upgrade). Gemini CLI uses `surface: cli` (project `.gemini/settings.json`; **re-trust** / `/hooks panel` / **folder trust**; AfterAgent cap ≤100 / `MAX_TURNS`; prefer CLI ≥0.31.0). Factory Droid uses `surface: cli` (project `.factory/hooks.json`; **`$FACTORY_PROJECT_DIR`**; **`/hooks` + snapshot/reload**; multi-block under `stop_hook_active` live-proved). Hermes Agent uses `surface: cli` (**`$HERMES_HOME/config.yaml`**; relative command; `pre_verify` continue live-proved; nudge ≥32; consent/non-TTY; soft min ≥0.21.3; **waive → degraded + R1 ack**). Antigravity uses `surface: cli` (project **`.agents/hooks.json`** + **`.agents/skills`** + **`.agents/bin` shim**; Stop `decision:continue`; PreInvocation via transcript; **host Stop continue live-proved** in 0.10.1). Devin CLI uses `surface: cli` (project **`.devin/hooks.v1.json`** + **`.devin/skills`**; **`$DEVIN_PROJECT_DIR`**; soft min **≥3000.10.31**; **`/hooks` + new session**; **CLI only** — not Desktop; **Shipped** — live CLI continue ≥1× proved). Pi uses `surface: cli` (in-process **`.pi/extensions/autopilot.ts`**; soft min **≥0.85.1**; **`/trust` + `/reload`**; **R10** interactive TUI only — **not** `pi -p` / JSON / print; shares **`.agents/skills`**; **not** a shell `--platform pi` stamp; **Shipped** — live TUI continue ≥1× proved). Runner uses `surface: runner` (no `hooks.json`; CLI `runner start|status` with **`--on`** / **`--run`** / bare resume; **`runner.command` required**; **not** a `--platform runner` hook stamp; **`one_executor`** still blocks dual armed executing; planning **`--brief`** is prompt-only).
...
packages/ports/devin         UserPromptSubmit / PostToolUse / Stop (aliased handleDevin*; Stop = decision:block+reason; $DEVIN_PROJECT_DIR; CLI only; Shipped)
packages/ports/pi            input / before_agent_start / tool_result / agent_settled (aliased handlePi*; continue = sendMessage+followUp+triggerTurn when pending; R10 TUI only; soft min ≥0.85.1; Shipped)
...
3. **Entry:** `.autopilot/bin/autopilot-harness-hook.mjs` imports `./vendor/runtime.mjs` and dispatches by **`--platform <id>`** (when present; **eleven-way** today) + host event + payload conflict resolver (cross-fire). **Pi** loads the same vendor from the in-process extension (no shell `--platform pi` stamp — matrix stays **eleven-way shell + Pi extension**).
...
| --- | --- | --- | --- |
| **Cursor** (shipped; **Tier-S** in **0.17**) | `hooks.json` `loop_limit` on **stop** / **subagentStop** | `5` if omitted | Autopilot **stop** already writes `"loop_limit": null` (`doctor` WARNs if missing). **0.17+:** also on Autopilot **`subagentStop`**; `doctor` WARNs if that entry is missing. Subagent policy is **neutral**; `subagentStop` **arms parent only** (no continue). See [hosts.md — Subagents](./hosts.md#subagents-neutral-policy--tiers). |
| **Claude Code** (v0.2 shipped; **Tier-S** in **0.17**) | Stop `decision: "block"` consecutive **block cap**; **`SubagentStop`** arm-only (**0.17+**) | **8**; `CLAUDE_CODE_STOP_HOOK_BLOCK_CAP` (`0` disables) | Init writes `.claude/settings.json` hooks + `env.CLAUDE_CODE_STOP_HOOK_BLOCK_CAP=0` (`doctor` WARNs when missing/not `0`). **0.17+:** also **`SubagentStop`**; `doctor` FAILs when that Autopilot fingerprint is missing. Workspace **trust** may gate project `env`. Same neutral / parent-arm semantics as Cursor. |
| **Codex** (shipped; **Tier-B**) | Stop `decision: "block"` + `reason` as next user prompt; `stop_hook_active` | No documented numeric block cap (2026-09 research) | **Shipped** hook port: `.codex/hooks.json` (omit timeout or ≥120s); `/hooks` trust + re-trust; P0 line-start `triggers.on` / `triggers.run` (no default skills/`AGENTS.md`; typed `/autopilot-*` still parses). **No** fake SubagentStop — parent **Stop dirty-arm** when child edits skipped the edit hook. |
...
| **Pi** (shipped) | `agent_settled` → `sendMessage` + `followUp` + `triggerTurn` (R1 pending-only) | No first-party hard Stop cap (0.85.1) | **Shipped** `@autopilot-harness/port-pi` — in-process **`.pi/extensions/autopilot.ts`**; soft min **≥0.85.1**; **`/trust` + `/reload`**; **R10** TUI only (not `pi -p` / JSON); live TUI continue ≥1× proved (edit arm yes; aim ≥2× not required for ship). Does not clamp `confirm_rounds`. |
| **Devin CLI** (shipped) | Stop `decision:"block"` + `reason`; `stop_hook_active` | No documented numeric Stop-continue cap / raise | **Shipped** `@autopilot-harness/port-devin` — **`.devin/hooks.v1.json`**; **`$DEVIN_PROJECT_DIR`**; skills **`.devin/skills`**; soft min **≥3000.10.31**; **`/hooks` + new session**; **CLI only** (not Desktop); live CLI continue ≥1× proved + edit arm. Does not clamp `confirm_rounds`. |
| **Runner** (shipped, meta) | External process loop; `runner.max_iterations` | Default **32** | **Shipped** `@autopilot-harness/port-runner` (public). CLI `runner start\|status`; **`--on`** / **`--brief`** / **`--message`** planning + **`--run`**; **`runner.command` required** (no fake default); **C6** planning `stopped`→exit **0** (`--run`/executing keep non-zero `stopped`); brief **not** persisted; dirty-arm + in-process `handleStop` (`platform:"runner"`). **Not** a `--platform runner` hook. **`one_executor`** still blocks dual armed executing. Live waive OK. |

**Tier-B** (Codex, Kimi, Copilot, Grok, Gemini, Factory, Hermes, Antigravity, Devin, …): **no** fake SubagentStop — parent **Stop dirty-arm** when child edits skipped the edit hook. Full table: [hosts.md — Subagents](./hosts.md#subagents-neutral-policy--tiers).

`beforeSubmitPrompt` / `afterFileEdit` (and Claude/Codex/Kimi/Copilot/Grok/Gemini/Factory/Hermes/Antigravity/Devin/Pi submit analogues —
`UserPromptSubmit` / `userPromptSubmitted` / `BeforeAgent` / `pre_llm_call` / `PreInvocation` / Pi `input` / `before_agent_start`)
