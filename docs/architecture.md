...
`platforms`, but **this build only installs Cursor** (`cursor`/`ide`) wiring;
other ids are reserved for future ports.
...
| --- | --- | --- | --- |
| **Cursor** (v0.1 shipped) | `hooks.json` `loop_limit` on **stop** / **subagentStop** | `5` if omitted | Write `"loop_limit": null` on Autopilot stop (`mergeHooksJson` / init / upgrade). `doctor` WARNs if missing. |
| **Claude Code** (v0.2 planned) | Stop `decision: "block"` consecutive **block cap** | **8**; `CLAUDE_CODE_STOP_HOOK_BLOCK_CAP` (`0` disables) | **Planned:** init sets `.claude/settings.json` `env.CLAUDE_CODE_STOP_HOOK_BLOCK_CAP=0`; `doctor` WARN when missing. Not wired in this Cursor-first build. |
