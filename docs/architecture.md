...
Autopilot Harness separates **core** (FSM, SQLite, checklist, review) from **ports** (Cursor, Claude Code, Codex, Kimi Code, GitHub Copilot CLI, Grok Build CLI, …).
...
`platforms`; **this build installs Cursor, Claude Code, Codex, Kimi Code, Copilot CLI, and/or Grok Build CLI** when those
bindings are present (`cursor`/`ide`, `claude-code`/`cli`, `codex`/`cli`, `kimi-code`/`cli`, `copilot-cli`/`cli`, `grok-build`/`cli`). Other ids are
...
Copilot CLI uses `surface: cli` (project `.github/hooks/autopilot-harness.json`; **Restart Copilot CLI** after install/upgrade). Grok Build CLI uses `surface: cli` (project `.grok/hooks/autopilot-harness.json`; **trust** via `/hooks-trust` or `--trust`; reload / new session after install/upgrade).
...
1. **Source of truth (CLI package):** `packages/cli/assets/vendor/`
   - `runtime.mjs` — esbuild bundle of core + **Cursor, Claude Code, Codex, Kimi Code, Copilot CLI, and Grok Build** ports (`pnpm bundle-vendor`; Codex/Kimi/Copilot/Grok handlers exported as `handleCodex*` / `handleKimi*` / `handleCopilot*` / `handleGrok*`)
...
**hard-caps** continuations (e.g. Kimi ≤1 / turn, Copilot consecutive ≤8, Grok ≤8 / turn), ship **degraded**
...
`beforeSubmitPrompt` / `afterFileEdit` (and Claude/Codex/Kimi/Copilot/Grok submit analogues —
